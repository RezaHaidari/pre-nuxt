...
function webpackBaseConfig(name) {
  ...
  if (extractCSS) {
    const extractOptions = Object.assign({ filename: this.options.build.filenames.css }, typeof extractCSS === 'object' ? extractCSS : {});
...
function webpackClientConfig() {
  ...
  if (!this.options.dev) {
    ...
    if (this.options.build.uglify !== false) {
      config.plugins.push(new UglifyJSPlugin(Object.assign({
        cache: true,
        sourceMap: true,
        parallel: true,
        extractComments: {
...
function styleLoader(ext, loaders = [], isVueLoader = false) {
  ...
  return [vueStyleLoader, cssLoader, postcssLoader, ...loaders, styleResourcesLoader].filter(l => l);
}
...
class Builder {
  ...
  get plugins() {
    return this.options.plugins.map((p, i) => {
      ...
      return {
        ...
        ssr: p.ssr !== false,
        name: path.basename(p.src, path.extname(p.src)).replace(/[^a-zA-Z?\d\s:]/g, '') + '_plugin_' + hash(p.src)
      };
    });
  }
  ...
  watchFiles() {
    ...
    const patterns = [r(src, 'layouts'), r(src, 'store'), r(src, 'middleware'), r(src, 'layouts/*.vue'), r(src, 'layouts/**/*.vue')];
    if (this._nuxtPages) {
      patterns.push(r(src, 'pages'), r(src, 'pages/*.vue'), r(src, 'pages/**/*.vue'));
    }
    ...
    this.customFilesWatcher = chokidar.watch(___default.uniq(this.options.build.watch), options).on('change', refreshFiles);
  }
...
class Generator {
  ...
  initRoutes() {
    ...
    return asyncToGenerator(function* () {
      ...
      if (_this3.options.router.mode !== 'hash') {
        try {
          generateRoutes = yield promisifyRoute(_this3.options.generate.routes || []);
        } catch (e) {
  ...
  initDist() {
    ...
    return asyncToGenerator(function* () {
      ...
      yield fs$1.remove(_this6.distPath);

      yield _this6.nuxt.callHook('generate:distRemoved', _this6);

      // Copy static and built files
      /* istanbul ignore if */
      if (fs$1.existsSync(_this6.staticRoutes)) {
        yield fs$1.copy(_this6.staticRoutes, _this6.distPath);
      }
      yield fs$1.copy(_this6.srcBuiltPath, _this6.distNuxtPath);
  ...
  generateRoute({ route, payload = {}, errors = [] }) {
    ...
    return asyncToGenerator(function* () {
      ...
      try {
        ...
        yield _this7.nuxt.callHook('generate:routeFailed', { route, errors: pageErrors });
      ...
      if (_this7.options.generate.subFolders) {
        ...
        path$$1 = route.length > 1 ? path.join(path.sep, route + '.html') : path.join(path.sep, 'index.html');
      }
      ...
      page.path = path.join(_this7.distPath, page.path);

      // Make sure the sub folders are created
      yield fs$1.mkdirp(path.dirname(page.path));
      yield fs$1.writeFile(page.path, page.html, 'utf8');

      yield _this7.nuxt.callHook('generate:routeCreated', { route, path: page.path, errors: pageErrors });
